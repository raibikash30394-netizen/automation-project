...
## v3.44 — DYNAMIC ORDER-SHIFT PROTECTION + solvedCaptcha crash fix (2026-02-12)
Two independent bugs from bhai's 2026-08-11 6th-run logs:

**BUG A — `ReferenceError: solvedCaptcha is not defined`** (WORKER_ERROR crash)
- Location: `handleBatch` captcha-invalidation path (line ~2265).
- Cause: variable was named `solved` (the function parameter) but the invalidation call referenced `solvedCaptcha`, causing a ReferenceError that the try/catch swallowed as generic WORKER_ERROR. Batch silently died; second-window submits (07:16, 07:46) never retried.
- Fix: all 3 call sites now use `solved` consistently (verified via grep).

**BUG B — Late-arriving orders locked out by `_planBuiltForWinKey` guard**
- User's complaint: "new oder ghuse ga tho kabhi kabhi wo uper ya niche kahi chala jata hai jise uska position change ho jata hai" (new orders shift position, get missed).
- Root cause: orders-poller built plan ONCE at T-1500ms then set winKey guard, blocking rebuild for the entire window. SAP publishes orders in a wide T-1500 → T+500ms window; late arrivals were in `_cachedOrders` but NOT in `_cachedPlan`. Cap-poller reused stale plan (age-guard passed) → late priority order silently ignored.
- Position-independence audit: matchOrder/buildBatches iterate lists and key purely by SapOrderId — the bot never assumed static positions. The bug was CACHE STALENESS, not positional matching.
- Fix:
  - `buildBatches` now returns `observedOrderIds` (Set<SapOrderId> at build time).
  - Orders-poller diffs current `_cachedOrders` vs. `observedOrderIds`; if new SapOrderIds exist → force rebuild (bypasses winKey guard).
  - Cap-poller does the same check before instant-dispatch; stale plan is discarded and rebuilt inline (~5-20ms).
  - `_planBuiltForWinKey` is RESET to 0 after cap-poller consumes the plan — so if SAP publishes yet another late order after the first submit, a fresh plan is built.
- New test: `testDynamicOrderShiftDetection` (5 cases) — validates late-arrival detection, position-shift-without-new-orders correctly does NOT rebuild, and cap-poller stale-plan discard logic.

**Verification**
- All 36 tests pass (`node tests/test-window-scheduler.js`).
- Syntax clean (`node -c bid-engine.js`).
- Smoke boot: engine starts, halts at expected "No cookie file found" gate — no runtime crashes.
