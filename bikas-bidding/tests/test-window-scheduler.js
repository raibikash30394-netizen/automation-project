...
// v3.44 — DYNAMIC ORDER-SHIFT PROTECTION.
//
// User's 2026-08-11 report: "new order ghusega toh kabhi kabhi wo uper ya
// niche kahi chala jata hai jise uska position change ho jata hai". SAP
// publishes orders in a narrow T-1500 → T+500ms window; a priority order
// arriving AFTER the initial plan-build (T-1500ms) was previously locked
// out because `_planBuiltForWinKey` guard prevented rebuild until the
// next :15/:45 boundary. v3.44 tracks observed SapOrderIds at plan-build
// time; when the orders-poller or cap-poller sees new SapOrderIds not
// in the snapshot, it forces a plan rebuild.
function testDynamicOrderShiftDetection() {
  // Simulate `buildBatches` returning observedOrderIds Set.
  const buildPlanSim = (orders) => {
    const observedOrderIds = new Set();
    for (const o of orders) {
      const sid = String(o?.SapOrderId || '');
      if (sid) observedOrderIds.add(sid);
    }
    return { observedOrderIds };
  };

  // Case A: no new orders → mustRebuild=false, havePreBuilt keeps plan.
  {
    const initial = [
      { SapOrderId: 'A1' }, { SapOrderId: 'A2' }, { SapOrderId: 'A3' },
    ];
    const snapshot = buildPlanSim(initial).observedOrderIds;
    let newCount = 0;
    for (const o of initial) {
      if (!snapshot.has(String(o.SapOrderId))) newCount++;
    }
    assert.strictEqual(newCount, 0, 'v3.44: identical order list must NOT trigger rebuild');
  }

  // Case B: 1 new order arrives after plan-build (real bhai scenario).
  {
    const initial = [
      { SapOrderId: 'A1' }, { SapOrderId: 'A2' }, { SapOrderId: 'A3' },
    ];
    ...
    assert.strictEqual(newCount, 1, 'v3.44: late-arriving A4 MUST trigger rebuild');
  }

  // Case C: SAP reorders the list but no new SapOrderIds (position-shift
  // without new orders — the bot doesn't care about positions since it
  // matches by SapOrderId, so rebuild NOT forced. Cache stays hot.).
  {
    const initial = [
      { SapOrderId: 'A1' }, { SapOrderId: 'A2' }, { SapOrderId: 'A3' },
    ];
    const snapshot = buildPlanSim(initial).observedOrderIds;
    const reordered = [
      { SapOrderId: 'A3' }, { SapOrderId: 'A1' }, { SapOrderId: 'A2' },
    ];
    let newCount = 0;
    for (const o of reordered) {
      if (!snapshot.has(String(o.SapOrderId))) newCount++;
    }
    assert.strictEqual(newCount, 0, 'v3.44: pure position-shift (no new SapOrderIds) does NOT trigger rebuild — SapOrderId matching is position-independent');
  }

  // Case D: multiple new priority orders arrive during hot-zone.
  {
    const initial = [
      { SapOrderId: 'A1' }, { SapOrderId: 'A2' },
    ];
    const snapshot = buildPlanSim(initial).observedOrderIds;
    const later = [
      { SapOrderId: 'A1' }, { SapOrderId: 'A2' },
      { SapOrderId: 'PRIO-1' }, { SapOrderId: 'PRIO-2' }, { SapOrderId: 'PRIO-3' },
    ];
    let newCount = 0;
    for (const o of later) {
      if (!snapshot.has(String(o.SapOrderId))) newCount++;
    }
    assert.strictEqual(newCount, 3, 'v3.44: 3 late priority orders must ALL be flagged for rebuild');
  }

  // Case E: cap-poller stale-plan guard (havePreBuilt=false when new orders).
  {
    const snapshot = new Set(['A1', 'A2']);
    const cachedOrders = [
      { SapOrderId: 'A1' }, { SapOrderId: 'A2' }, { SapOrderId: 'A3-LATE' },
    ];
    let newOrdersSincePlan = 0;
    for (const o of cachedOrders) {
      const sid = String(o?.SapOrderId || '');
      if (sid && !snapshot.has(sid)) newOrdersSincePlan++;
    }
    const planAge = 800; // fresh plan
    const havePreBuilt = 3 > 0 && planAge <= 10_000 && newOrdersSincePlan === 0;
    assert.strictEqual(havePreBuilt, false, 'v3.44 cap-poller: stale plan (new orders since build) must NOT be reused');
    assert.strictEqual(newOrdersSincePlan, 1, 'v3.44 cap-poller: exactly 1 late order detected');
  }
